      ...
      if(response.isOk)
        {
          ...
          if(apiResponse.success==true) {
            ...
            SnackBarHelper.showErrorSnackBar('Failed To Delete Notification${apiResponse.message}');
          }
        }
      else{
  ...
  getNotificationInfo(MyNotification? notification)async{
    try{
      if(notification==null) {
        SnackBarHelper.showErrorSnackBar('Notification is null');
        return;
      }
      final response=await service.getItems(endpointUrl: 'notification/track-notification/${notification.sId}');
      if(response.isOk){
        final ApiResponse<NotificationResult?>apiResponse=ApiResponse<NotificationResult?>.fromJson(response.body, (json)=>
        ...
        if(apiResponse.success==true){
          ...
          SnackBarHelper.showErrorSnackBar('Failed To Fetch Notification${apiResponse.message}');
        }
      }
      else{
        SnackBarHelper.showErrorSnackBar('error: ${response.body?['message']?? response.statusText}');
