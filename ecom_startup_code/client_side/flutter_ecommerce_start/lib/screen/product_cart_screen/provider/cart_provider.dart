...
class CartProvider extends ChangeNotifier {
  ...
  checkCoupon() async{
    try{
      if(couponController.text.isEmpty)
        {
          SnackBarHelper.showErrorSnackBar('Enter Coupon Code');
          return;
        }
      ...
      if(response.isOk) {
        ...
        if (apiResponse.success == true) {
          ...
          SnackBarHelper.showErrorSnackBar('Failed to apply Coupon: ${apiResponse.message}');
        }
      }
      else{
  ...
  double getCouponDiscountAmount(Coupon coupon)
  {
    ...
    {
      double percentage=coupon.discountAmount??0;
      double discountAmount=(percentage/100)*getCartSubTotal();
      return discountAmount;
    }
