...
class DataProvider extends ChangeNotifier {
  ...
  DataProvider() {
    ...
    getAllProduct();
    getAllPosters();
    getAllOrderByUser(GetStorage().read(USER_INFO_BOX));
  ...
  void filterSubCategory(String keyword) {
    if(keyword.isEmpty) {
      _filteredSubCategories = List.from(_allSubCategories);
    }
    else{
  ...
  Future<void> getAllOrderByUser(User? user, {bool showSnack = false}) async {
    try {
      ...
      if (userId == null) {
        if (showSnack) SnackBarHelper.showErrorSnackBar('Invalid user ID');
        return;
      }

      final response = await service.getItems(endpointUrl: 'orders/orderByUserId/$userId');

      if (response.isOk) {
        final ApiResponse<List<Order>> apiResponse = ApiResponse<List<Order>>.fromJson(
          response.body,
        ...
        if (showSnack) {
          SnackBarHelper.showErrorSnackBar(
            'Failed to fetch orders: ${response.body?['message'] ?? response.statusText}',
          );
        }
      }
    } catch (e) {
      if (showSnack) SnackBarHelper.showErrorSnackBar('An error occurred: $e');
